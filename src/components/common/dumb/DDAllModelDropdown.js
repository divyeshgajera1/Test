...
const DDAllModelDropdown = ({ label, input, data, onSelected, disabled,inputstyle }) => {
  ...
  const onChangeSearch = (query) => {
    ...
    const filterData = data
      .filter((item) => {
        const desc = item.MODEL.toLowerCase()
  ...
  const renderItem = ({ item }) => (
    <ListItem
      ...
      bottomDivider
      onPress={() => {
        onSelected(item.MODEL, item.MODELID, item.MAKEID)
        toggleOverlay()
      }}
    />
  )
...
export default DDAllModelDropdown
